...
for arquivo in os.listdir(r'C:\Users\ferna\Desktop\bolsaMachine\carrinhotrocarLossfunction\pairStatesActions'):
       with open('C:/Users/ferna/Desktop/bolsaMachine/carrinhotrocarLossfunction/pairStatesActions/' + arquivo, 'rb') as file:
              data = pkl.load(file)
       dataSet.append(data)
...
trainingY = np.array(Y[:limit]).astype('float32')
validationX = np.array(X[limit:]).astype('float32')
validationY = np.array(Y[limit:]).astype('float32')
...
train_loader = torch.utils.data.DataLoader(trainingSet, batch_size=512) #, drop_last=True

""" training parameters """
optimizer = optim.Adam(CNN.parameters(), lr=0.0001)
criterionBCE = nn.BCELoss()
criterionHuber = nn.HuberLoss()
epochs = 350
limitEpochs = 3
...
for epoch in range(epochs):
       ...
       for batch in train_loader:
              ...
              loss = criterionHuber(predictions[:,0],target[:,0]) + criterionBCE(predictions[:,1],target[:,1]) + criterionBCE(predictions[:,2],target[:,2])
              optimizer.zero_grad()
              loss.backward()
              optimizer.step()
       ...
       with torch.no_grad():
              ...
              trainTarget = torch.squeeze(trainTarget,1)
              trainInput, trainTarget = trainInput.to(device), trainTarget.to(device)
              trainPredictions = CNN(trainInput)
              trainLoss = criterionBCE(trainPredictions[:,0],trainTarget[:,0]) + criterionBCE(trainPredictions[:,1],trainTarget[:,1]) + criterionBCE(trainPredictions[:,2],trainTarget[:,2])
              ...
              valTarget = torch.squeeze(valTarget,1)
              valInput, valTarget = valInput.to(device), valTarget.to(device)
              valPredictions = CNN(valInput)
              valLoss = criterionHuber(valPredictions[:,0],valTarget[:,0]) + criterionBCE(valPredictions[:,1],valTarget[:,1]) + criterionBCE(valPredictions[:,2],valTarget[:,2])
